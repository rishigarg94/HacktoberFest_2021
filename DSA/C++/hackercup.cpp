...
map<ll int,ll int> printCommonElements(vector<vector<ll int>>mat)
{
    ...
    m=mat[0].size();
    set<ll int> ans;
    map<ll int,ll int > temp;
    for(int i=0;i<m;i++){
        
        temp[mat[0][i]]++;
        freq[mat[0][i]]=temp[mat[0][i]];
    }
...
ll int furthersolve(vector<vector<ll int>>p,map<ll int,ll int>common,ll int n,ll int m){
    map<ll int,ll int> last;
    ll int ans=0;
    ll int safe=0;
    ll int same=0;
    for(auto x:common){
    ...
    for(ll int i=1;i<n;i++){
        map<ll int,ll int> curr;
        for(ll int j=0;j<m;j++){
            curr[p[i][j]]++;
        }
        
        map<ll int,ll int> change;
        for(auto x:last){
            
            if(curr.find(x.first)!=curr.end()){
                if(curr[x.first]<last[x.first]){
                    change[x.first]=last[x.first]-curr[x.first];
                }
            }
            else{
                change[x.first]=last[x.first];
            }
        }
        ...
        if(i!=1){
            for(auto x:change){
                if(common.find(x.first)!=common.end()){
                    ll int tt=max(0*1ll,(common[x.first]-change[x.first]));
                    safe+=common[x.first]-tt;
                    common[x.first]=tt;
    ...
    ans-=safe;
    ans-=same;
    return max(0*1ll,ans);
    

}
void solve(int TN){
    //cout<<" i am at tn equals "<<TN+1<<endl;
    ...
    cin>>n>>m;
    //cout<<" n is "<<n<<" m ins "<<m<<endl;
    
    
    
    vector<vector<ll int>>p(n+1,vector<ll int>(m,0));
    /*for(int i=0;i<=n;i++){
        print_vec_int(p[i]);
    ...
    map<ll int,ll int>common=printCommonElements(p);
    cout<<" common elements are :"<<endl;
    for(auto x:common){
        cout<<x.first<<" : "<<x.second<<endl;
    }
    cout<<" common ends \n";
    cout<<furthersolve(p,common,n+1,m)<<endl;
